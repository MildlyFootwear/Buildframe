        ...
        private void FormStatWizard_Load(object sender, EventArgs e)
        {
            ...
            foreach (Control c in tableLayoutPanel1.Controls)
            {
                if (c is NumericUpDown nud)
                {
                    ...
                    int tabIndex = tableLayoutPanel1.GetColumn(nud) * 100 + tableLayoutPanel1.GetRow(nud); // Blame me for this later.
            ...
            foreach (Control c in tableLayoutPanel3.Controls)
            {
                if (c is NumericUpDown nud)
                {
                    ...
                    nud.ValueChanged += numericUpDown_TrackChanged;
                    int tabIndex = tableLayoutPanel3.GetColumn(nud) * 100 + tableLayoutPanel3.GetRow(nud); // Blame me for this later.
                    nud.TabIndex = tabIndex;
                }
            }
            ...
            foreach (Control c in tableLayoutPanel4.Controls)
            {
                if (c is NumericUpDown nud)
                {
                    ...
                    nud.ValueChanged += numericUpDown_TrackChanged;
                    int tabIndex = tableLayoutPanel4.GetColumn(nud) * 100 + tableLayoutPanel4.GetRow(nud); // Blame me for this later.
                    nud.TabIndex = tabIndex;
                }
            }
